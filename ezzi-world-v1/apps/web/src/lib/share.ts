...
export async function generateShareCard({
  ...
  if (type === 'nft') {
    ...
    ctx.fillStyle = getRarityColor(data.rarity);
    ctx.fillText(data.name, 600, 300);

    // Rarity
    ctx.font = '40px Inter';
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`${data.rarity.toUpperCase()}`, 600, 380);
    ...
    ctx.fillText(
      data.resultType === 'nft' ? data.nftName : `${data.amount} EZZI`,
      600,
      300
    );
  } else if (type === 'mining') {
    ...
    ctx.fillText(`${data.amount} EZZI`, 600, 300);

    // Time
    ctx.font = '40px Inter';
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`in ${data.hours} hours`, 600, 380);
  }
...
// Share on Twitter
export function shareOnTwitter(text: string, imageUrl?: string) {
