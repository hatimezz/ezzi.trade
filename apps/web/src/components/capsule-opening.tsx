...
interface CapsuleOpeningProps {
  capsuleId: string;
  capsuleName: string;
  onComplete: (result: CapsuleResult) => void;
  onClose: () => void;
}

export function CapsuleOpening({ capsuleId, onComplete, onClose }: CapsuleOpeningProps) {
        ...
        {phase >= PHASES.REVEAL && result && (
          ...
          >
            {result.resultType === 'nft' ? (
              ...
              <EZZIResult amount={result.amount ?? 0} />
            )}
...
function NFTResult({ result }: { result: CapsuleResult }) {
