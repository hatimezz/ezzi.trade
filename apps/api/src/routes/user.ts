...
router.get('/nfts', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('[USER NFTS ERROR]', error);
...
router.post('/connect-wallet', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('[CONNECT WALLET ERROR]', error);
    res.status(500).json({
      success: false,
...
// GET /api/users/capsules - Get user's capsule openings
router.get('/capsules', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;

    const capsules = await prisma.capsuleOpening.findMany({
      where: { userId },
      include: {
        capsuleTier: {
          select: {
    ...
    logError('[USER CAPSULES ERROR]', error);
